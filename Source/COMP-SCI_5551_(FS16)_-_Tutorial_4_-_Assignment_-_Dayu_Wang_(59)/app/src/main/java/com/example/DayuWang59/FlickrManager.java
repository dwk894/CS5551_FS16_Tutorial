...
class FlickrManager {
	...
	private static final String FLICKR_BASE_URL = "https://api.flickr.com/services/rest/?method="; //Using SSL -> so "https".
	...
	private static final String APIKEY_SEARCH_STRING = "&api_key=bd5aab124fd7b84656a32cc5b35f98e9"; //Dayu Wang's API Key
	
	private static final String TAGS_STRING = "&tags=";
	private static final String PHOTO_ID_STRING = "&photo_id=";
	private static final String FORMAT_STRING = "&format=json&nojsoncallback=1"; //JsonP is NOT Json!
	static final int PHOTO_THUMB = 111;
	static final int PHOTO_LARGE = 222;
	...
	static Bitmap getImage(ImageContener imgCon) {
		Bitmap bm = null;
		try {
	...
	private static Bitmap getThumbnail(ImageContener imgCon) {
		Bitmap bm = null;
		try {
	...
	static class GetThumbnailsThread extends Thread {
		UIHandler uih;
		ImageContener imgContener;

		GetThumbnailsThread(UIHandler uih, ImageContener imgCon) {
			this.uih = uih;
	...
	static ArrayList<ImageContener> searchImagesByTag(UIHandler uih, Context ctx, String tag) {
		uihandler = uih;
